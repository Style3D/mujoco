...
typedef struct mjVisual_ {        // visualization options
  struct mjGlobal{                        // global parameters
  ...
  struct mjQuality{                        // rendering quality
  ...
  struct mjScale{                        // scale of decor elements relative to mean body size
  ...
  struct mjRgba{                        // color of decor elements
    float fog[4];                 // fog
    float haze[4];                // haze
...
typedef struct mjModel_ {
  ...
  mjtBool*  flex_custom;          // use custom simulator instead of mujoco   (nflex x 1)
